...
--------------------------------------------------------------------------------------
    ...
    identical_title  same employer, same title, ordered  → PROPOSED, operator decides
    fuzzy_title      same employer, similar title        → PROPOSED, operator decides

Only the requisition tier merges on its own, because only it is evidence rather than inference.
Everything else enqueues a `JobMatch` for review. This is the same rule `applied_index` already
lived by, and the reason is unchanged: **a near-miss that silently hides a job the operator
picked is worse than one that asks.** The review queue is cheap; an invisible job is not.
...
AUTO_MERGE_TIERS = frozenset({"requisition"})
...
def compare(a: Job, b: Job) -> Optional[MatchProposal]:
    ...
    platforms = sorted(set(a.source_platforms or []) | set(b.source_platforms or []))
    if len(platforms) > 1:
        evidence.append(f"seen on {', '.join(platforms)}")

    if title_tokens(a.title) == title_tokens(b.title):
        evidence.insert(0, f"identical title ({keep.title})")
        # Same title AND same stated location is as strong as inference gets here. It still asks,
        # because a large employer really does post the same title twice in two offices.
        if _same_location(a, b):
            evidence.append("same location")
        return MatchProposal(keep.job_key, fold.job_key, "identical_title", score, evidence)
...
def _same_location(a: Job, b: Job) -> bool:
    la, lb = normalize_company(a.location), normalize_company(b.location)
...
def resolve_sighting(db: Session, sighting: ObservedJob) -> Job:
    ...
    for cand in _company_candidates(db, company_norm, reqs):
        ...
        if cand.company_norm == company_norm and title_tokens(cand.title) == title_tokens(sighting.title):
            if not level_ranks(cand.title) or level_ranks(cand.title) == level_ranks(sighting.title):
                return _attach(db, sighting, cand)
...
def backfill(db: Session) -> dict[str, int]:
    ...
    Three passes, in this order for a reason: a sighting must have a job before it can have an
    application, and duplicates must be found after every job exists or the scan only sees half
