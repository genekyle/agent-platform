...
const TONE = { ok: "success", error: "danger", info: "neutral", warn: "warning" };
  ...
  return (
    <div className="layer">
      <div className="layer__head">
        <div className="layer__title layer__title--with-icon"><AppIcon name="activity" size={17} /> {title}</div>
      </div>
      {rows.length === 0 ? (
        ...
        rows.map((e, i) => {
          ...
          return (
            <div key={e.id || i} className="activity-row">
              <span className={`dot activity-dot activity-dot--${TONE[e.status] || "neutral"}`} />
